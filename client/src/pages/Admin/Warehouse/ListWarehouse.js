...
function ListWarehouse() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_URL_API}/warehouses?keyword=${keyword}&page=${page}&limit=${limit}`)
      .then((response) => {
  ...
  const deleteWarehouse = (Id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_URL_API}/warehouses/${Id}`, {
            headers: {
  ...
  return (
    <>
      ...
      <Card elevation={4}>
        <AdminCardHeader list title={"warehouse"} to={"/admin/warehouse"}>
                ...
                <TableBody>
                  {list.map((item, index) => (
                    <TableRow key={item.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                      ...
                      <TableCell align="center">
                        <Link to={`/admin/editwarehouse/${item.id}`}>
