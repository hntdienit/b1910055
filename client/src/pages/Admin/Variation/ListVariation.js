...
import { faAdd, faEye, faHouse, faMagnifyingGlass, faPen, faTrash } from "@fortawesome/free-solid-svg-icons";
...
function ListCategory() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_URL_API}/categories?page=${page}&limit=${limit}`).then((response) => {
          ...
          <div className={cl("d-flex align-items-center")}>
            ...
            <form className={cl("ms-auto position-relative")}>
              <div
                ...
                className={cl("search-icon", "position-absolute top-50 translate-middle-y px-3")}
              >
                <FontAwesomeIcon icon={faMagnifyingGlass} className={""} />
              </div>
              <input className={cl("form-control ps-5")} type="text" placeholder="Tìm kiếm...." />
            </form>
            <Button
              to={"/admin/category"}
                      ...
                      <td>
                        <div className={cl("table-actions d-flex align-items-center gap-3 fs-6")}>
                          ...
                          <div className={cl("")} title="Edit">
                            <button>
                              <Link to={`/admin/editcategory/${value.id}`} className={cl("text-warning")}>
