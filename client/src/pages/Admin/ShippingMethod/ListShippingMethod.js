...
function ListShippingMethod() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_URL_API}/shippingmethods?keyword=${keyword}&page=${page}&limit=${limit}`)
      .then((response) => {
  ...
  const deleteItem = (Id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_URL_API}/shippingmethods/${Id}`, {
            headers: {
  ...
  return (
    <>
      ...
      <Card elevation={4}>
        <AdminCardHeader list title={"Shipping Method"} to={"/admin/shippingmethod"}>
                ...
                <TableBody>
                  {list.map((item, index) => (
                    <TableRow key={item.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                      ...
                      <TableCell align="center">
                        <Link to={`/admin/editshippingmethod/${item.id}`}>
